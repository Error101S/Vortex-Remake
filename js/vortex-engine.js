...
function studTex(rx, ry) {
    const t = tlLoader.load('assets/textures/stud.jpeg');
...
function getCachedMats(sw, sh, sd, color) {
    ...
    const m = (rx, ry) => new THREE.MeshLambertMaterial({ color, map: studTex(rx, ry) });
    const mats = [
...
renderer.domElement.addEventListener('wheel', e => {
    if (locked) {
        ...
        if (msgsEl) {
            const r = msgsEl.getBoundingClientRect();
...
function updateAnimations(dt, moving) {
    ...
    if (!grounded) {
        ...
        // ── Idle: slowed-down walk cycle (matches Roblox amplitude=0.1, freq=1) ──
        const idleFreq = 1.0;
        const idleAmp  = 0.1;
        const swing    = Math.sin(t * idleFreq) * idleAmp;

        // Right shoulder mirrors left shoulder (rs = -ls in Roblox),
        // right hip mirrors left hip (rh = -lh).
        setRot(lLeg,  'x',  swing,   sp, dt);   // lh  =  desiredAngle
        setRot(rLeg,  'x', -swing,   sp, dt);   // rh  = -desiredAngle
        setRot(lArm,  'x', -swing,   sp, dt);   // ls  = -desiredAngle
        setRot(rArm,  'x',  swing,   sp, dt);   // rs  =  desiredAngle
        setRot(lArm,  'z',  0.1,     sp, dt);
        setRot(rArm,  'z', -0.1,     sp, dt);
        setRot(torso, 'x',  0,       sp, dt);
        setRot(torso, 'z',  0,       sp, dt);
        if (lArm) lArm.position.y = THREE.MathUtils.lerp(lArm.position.y, lArmRestY, Math.min(1, sp * dt));
        if (rArm) rArm.position.y = THREE.MathUtils.lerp(rArm.position.y, rArmRestY, Math.min(1, sp * dt));
...
function resolveBlocksH(nearby) {
    ...
    for (const b of nearby) {
        ...
        if (ov0 <= ov1) {
            character.position.x -= Math.sign(dx) * ov0;
        } else {
            character.position.z -= Math.sign(dz) * ov1;
        }
        pushedBlocks.add(b);
    }
}
